...
以上尖括号内容是占位符，不能原样写入索引，也不要虚构 Workshop ID。
...
创作者投稿流程、必填证据和本地命令见 [CONTRIBUTING.md](CONTRIBUTING.md)。PR 工作流 `.github/workflows/validate-index.yml` 会在 Windows 上构建项目、运行回归，并通过 CLI 调用同一生产验证器；运行时验证仍是最终安全边界。
...
- 重复 Mod ID、重复规范化 Workshop ID、非法/null 条目会拒绝整份索引；
- `ModManager.exe` 内嵌 Bridge、提取、SHA-256 校验和损坏后修复；
- MainForm 的基线/下一次启动/手动关闭文案与 guide、banner、列表、状态栏布局；
- ModCard 忙碌状态与工坊/旧直装按钮回归。

`--validate-index <path>` 成功时返回 `0` 并输出条目数；解析、格式、重复或 I/O 失败时输出错误并返回非零。CI 必须使用该入口，避免维护第二套与运行时分叉的校验逻辑。
